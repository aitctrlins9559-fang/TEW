...
export const VersionHistoryModal: React.FC<VersionHistoryModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div className="glass-card w-full max-w-2xl rounded-3xl border border-sky-500/30 shadow-2xl p-6 md:p-8 space-y-6 max-h-[85vh] flex flex-col overflow-hidden">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                版本更新紀錄 <span className="text-xs px-2 py-0.5 rounded-full bg-sky-500/20 text-sky-300 font-mono">Changelog</span>
              </h2>
              <p className="text-xs text-slate-400">系統版本演進歷程與最新功能說明</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        {/* Content - Scrollable */}
        <div className="overflow-y-auto pr-1 space-y-6 flex-1 divide-y divide-white/5">
          {VERSION_LOGS.map((item, idx) => (
            <div key={idx} className={`${idx !== 0 ? 'pt-6' : ''} space-y-3`}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  <span className="text-sm font-black font-mono text-sky-400 bg-sky-500/10 border border-sky-500/30 px-2.5 py-1 rounded-xl">
                    {item.version}
                  </span>
                  {item.badge && (
        ...
        {/* Footer */}
        <div className="pt-4 border-t border-white/10 flex justify-between items-center text-xs text-slate-400 shrink-0">
          <div className="flex items-center gap-1 text-slate-400">
            <ShieldCheck className="w-4 h-4 text-emerald-400" /> 持股監控投資雷達 • 穩定極速體驗
          </div>
          <button
            onClick={() => {
              playClickSound();
            ...
            className="bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold px-5 py-2 rounded-xl transition btn-interact"
          >
            瞭解並關閉
          </button>
        </div>
