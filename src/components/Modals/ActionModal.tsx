...
export const ActionModal: React.FC<ActionModalProps> = ({
  ...
  if (isMVP) {
    ...
    else extraText = '穩健獲利中！🛡️ 保持紅字就是舒服，不疾不徐累積資產！';
  } else {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
      <div className="glass-card rounded-3xl p-8 w-full max-w-sm shadow-[0_0_50px_rgba(56,189,248,0.2)] text-center border border-white/10">
        <div
          className={`w-20 h-20 mx-auto rounded-full flex items-center justify-center mb-6 shadow-inner border ${
            isMVP
              ? 'bg-emerald-500/20 border-emerald-500/30 animate-bounce'
        ...
        <h3
          className={`text-2xl font-black mb-2 tracking-tight ${
            isMVP ? 'text-emerald-400' : 'text-rose-400'
          }`}
        >
          {isMVP ? '獲利成就解鎖！' : '風控冷靜室'}
        </h3>
        ...
        <div className="text-slate-300 text-sm mb-8 leading-relaxed font-medium">
          <div
            className={`text-[10px] mb-3 font-mono tracking-widest uppercase border inline-block px-2 py-0.5 rounded ${
              isMVP
                ? 'text-emerald-400/80 border-emerald-500/20 bg-emerald-500/10'
                : 'text-rose-400/80 border-rose-500/20 bg-rose-500/10'
            }`}
          >
